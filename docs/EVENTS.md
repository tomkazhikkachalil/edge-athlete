...
| `sport-events-contest` (phase 2b, NEEDS 211 — self-skips before) | a QA club with a golf net league + a hockey fixture · a two-round club event · PUT contest mints one contest per round on the rounds' dates with A entered · idempotent · the golf-sync engine refuses an event round's contest · the hockey competition → 400 `not_golf_leaderboard` · B → 403 · null unlinks |
