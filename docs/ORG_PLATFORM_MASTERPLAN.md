...
| 6 | ad-hoc sides (the first 219 reader): `entries.ts` (`entryDisplayName` at every reader, `adHocEntryRefusal`), `EntryAddSchema {name, memberProfileIds}`, the ad-hoc branch of `entryAddPOST` (members = the org roster), `ad_hoc_team` on offer in the profiles, the console's entrant-kind select + "Add a team from players", the stat-lines gate widened to a side's members |
| 7 | the meet (zero DDL): `meet.ts` (marks parsed by name, `placeEvent` by the event's direction with a DQ unranked, `computeMeetStandings` — the points of the places summed per AFFILIATION, medals break ties, `parseMeetConfig`), the meet outcome (a leaderboard contest with the event's rule injected by the round label), the standings' meet branch (`readMeetRows`, `ensureMeetTeamEntries` — the roll-up rows are TEAM entries), `meetEventsGeneratePOST` (one contest per event: round = the label, stage = the session, slot = the order; never published), `meetResultsUpsertPOST` (results + ONE stat line per athlete per event → the performance row; the contest completes), the affiliation snapshot at entry + `PATCH entries {affiliationTeamId}`; meet creatable |
...
> checklist). Phase 4 folded into 3 and 5. **Phases 9–11 (Sep 9, #637–#643,
> zero DDL):** widgets bound to a query (`config.query`; standings / schedule /
> leaders may repeat); the in-app page follows the composition (order,
> titles, content tiles) and the editor is the Website section's door (the
