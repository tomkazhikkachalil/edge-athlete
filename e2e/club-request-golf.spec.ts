...
test('golf fast path: Golf pre-checked → home course prefills → two steps → site_draft truth; 375px', async ({
  ...
  try {
    ...
    // The door's defaults: Golf checked; the directory choice defaults to
    // listed-after-review; the capabilities question is GONE from the small
    // path (R2 — derived: a golf club runs competitions).
    await expect(pageB.getByRole('checkbox', { name: 'Golf' })).toBeChecked();
    await expect(pageB.getByRole('radio', { name: /In the directory/ })).toBeChecked();
    await expect(pageB.getByRole('checkbox', { name: /We run competitions/ })).toHaveCount(0);

    // The home course (optional) lives under "More details": search → pick → prefills.
    await pageB.getByRole('button', { name: /More details/ }).click();
    ...
    await expect(pageB.getByText('Live now · listing under review')).toBeVisible();
    ...
    // R2: the org is live — the wizard hands off straight into the console.
