...
test('the match bridge: a bracket final runs as a match, go-live swaps the link, completion writes the winner and crowns the champion', async () => {
  ...
  const userC = s.userC!;
  const stamp = s.stamp;
  const league = await createQaOrg(admin, 'league', { name: `QA Match Bridge ${stamp}`, sport_key: 'golf', owner_profile_id: s.userA.id, visibility: 'public' });
  const leagueId = league.id;
  let eventId: string | null = null;
  try {
    await admin.from('memberships').insert([
      { org_id: leagueId, profile_id: s.userA.id, kind: 'follow', role: 'owner', status: 'active', scope_type: 'org', scope_id: null },
      ...[s.userB.id, userC.id].map(profile_id => ({ org_id: leagueId, profile_id, kind: 'roster', role: 'member', status: 'active', scope_type: 'org', scope_id: null })),
    ]);
