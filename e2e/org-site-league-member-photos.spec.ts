...
// Program 12 — league round photos, part 2 (the twin of
// org-site-member-photos): a picked photo renders on a LEAGUE site's
// public gallery through the gated tokenless streamer; the gate re-runs
// per request: a consent revoke, the post made private, or the league
...
test('league member photos on the site: gallery tile + streamer; revoke → 404; private post → 404; private league → 404; 375px', async ({
  ...
  try {
    ...
    res = await ownerApi.patch(`/api/leagues/${leagueId}/site`, { data: { action: 'set_gallery_pick', mediaId: seed.mediaId } });
    expect(res.status(), await readErrorBody(res)).toBe(200);
    ...
    // Vercel's CDN keeps a streamer 200 for its short s-maxage — every
    // post-change check asks a FRESH URL so the gate itself answers.
    ...
    res = await alphaApi.patch(`/api/leagues/${leagueId}/photo-consent`, { data: { consent: false } });
    expect(res.status(), await readErrorBody(res)).toBe(200);
    ...
    // The league gone private → 404 even with the pick (the phase-9 rail, 177).
    res = await ownerApi.patch(`/api/leagues/${leagueId}`, { data: { visibility: 'private' } });
    expect(res.status(), await readErrorBody(res)).toBe(200);
    expect((await anon.request.get(fresh())).status()).toBe(404);
