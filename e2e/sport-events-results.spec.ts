...
type Rounds = { rounds: Array<{ id: string; course: string; holes: number; gross_score: number }> };
...
test('sport events API: leaderboard · results mirror · opt-out', async () => {
  ...
  try {
    ...
    // B opts out → the mirror is gone; opts back in → it is back. The organizer cannot flip it.
    ...
    expect(afterHide.rounds.find(r => r.course === courseName)).toBeUndefined();
    ...
    expect(afterShow.rounds.find(r => r.course === courseName)).toMatchObject({ gross_score: 9 });
    ...
    expect((await apiA.delete(`/api/sport-events/${eventId}`)).ok()).toBe(true);
    eventId = null;
  } finally {
    if (eventId) {
