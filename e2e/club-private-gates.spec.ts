...
// private cache) and the in-app club page; the club GET hides the member
...
test('private club: panels on the site, empty public standings, members read /mine, outsiders see no names, search chip, sitemap, flip back; 375px', async ({
  browser,
}) => {
  ...
  try {
    ...
    await expect
      ...
      .toBe(200);
    expect(home).toContain(`QA Private Club ${stamp}`);
    expect(home).toContain('data-members-only="1"');
    expect(home).toContain(`/join/club/${clubId}"`);
    noNames(home, 'home');
    ...
    const pub = (await (await anon.request.get(`/api/clubs/${clubId}/standings?_cb=${Date.now()}`)).json()) as { competitions: unknown[] };
    expect(pub.competitions).toEqual([]);
    const twin = await (await anon.request.get(`/club/${clubId}/standings`)).text();
    expect(twin).toContain('No published standings yet');
    noNames(twin, 'twin');

    // Members read /mine; outsiders don't.
    expect((await anon.request.get(`/api/clubs/${clubId}/standings/mine`)).status()).toBe(401);
    expect((await strangerApi.get(`/api/clubs/${clubId}/standings/mine`)).status()).toBe(403);
    res = await alphaApi.get(`/api/clubs/${clubId}/standings/mine`);
    expect(res.status(), await readErrorBody(res)).toBe(200);
    ...
    // The club GET: no member list for outsiders; the list for members.
    const outsiderView = (await (await strangerApi.get(`/api/clubs/${clubId}`)).json()) as { members: unknown[]; memberCount: number; visibility: string };
    expect(outsiderView.members).toEqual([]);
    expect(outsiderView.memberCount).toBe(2);
    expect(outsiderView.visibility).toBe('private');
    const memberView = (await (await alphaApi.get(`/api/clubs/${clubId}`)).json()) as { members: unknown[] };
    expect(memberView.members.length).toBeGreaterThan(0);

    // Search marks it private.
    const search = (await (await ownerApi.get(`/api/search?q=${encodeURIComponent(`QA Private Club ${stamp}`)}&type=clubs`)).json()) as { results?: { clubs?: { id: string; visibility?: string }[] }; clubs?: { id: string; visibility?: string }[] };
    const found = (search.results?.clubs ?? search.clubs ?? []).find(c => c.id === clubId);
    ...
    // The in-app club page for a member shows the standings (via /mine) at 375px.
    ...
    try {
      ...
      await expect(mp.getByText(`Private League ${stamp}`).first()).toBeVisible({ timeout: 20_000 });
