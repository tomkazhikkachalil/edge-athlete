// ── One handler for both kinds (Round 5 E-2): the body of /api/{leagues,clubs}/[id]/site/news ──
...
// ── /api/{leagues,clubs}/[id]/site/news — news list + create (phase 3 R3) ─────────
// manage_org gates it; writes ride the org-site-pages bucket (editor
// saves must not burn the 30/h org-site budget).

export async function siteNewsRouteGET(request: NextRequest, kind: OrgKind, params: { id: string }) {
  try {
    ...
    return await newsListGET(admin, kind, id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError(`[ORG SITE NEWS] ${kind} GET error:`, error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function siteNewsRoutePOST(request: NextRequest, kind: OrgKind, params: { id: string }) {
  try {
    const user = await requireAuth(request);
