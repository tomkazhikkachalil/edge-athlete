// ── One handler for both kinds (Round 5 E-2): the body of /api/{leagues,clubs}/[id]/competitions/[competitionId]/schedule-import ──
...
// ── /api/{leagues,clubs}/[id]/competitions/[competitionId]/schedule-import ────────────
...
export async function competitionsOneScheduleImportRoutePOST(request: NextRequest, kind: OrgKind, params: { id: string; competitionId: string }) {
  try {
    ...
    const { data: comp } = await admin
      .from('competitions')
      .select('id, format, status, org_id, org:organizations(kind)')
      .eq('id', competitionId)
      .maybeSingle();
    ...
    const scope = { side: kind, orgId: id };
    return ics
    ...
    reportRouteError(`[SCHEDULE-IMPORT] ${kind} POST error:`, error);
