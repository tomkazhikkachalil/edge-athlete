// ── One handler for both kinds (Round 5 E-2): the body of /api/{leagues,clubs}/[id]/structure-import ──
...
// ── /api/{leagues,clubs}/[id]/structure-import (phase 6 R5) ─────────────────────────
// Divisions + teams + entries by CSV paste, dry-run-first, idempotent by
// constraint. Manager-gated; the roster-import sibling.

export async function structureImportRoutePOST(request: NextRequest, kind: OrgKind, params: { id: string }) {
  try {
    ...
    return await structureImportPOST(admin, kind, id, (org?.sport_key as string | null) ?? null, {
    ...
    reportRouteError(`[STRUCTURE-IMPORT] ${kind} POST error:`, error);
