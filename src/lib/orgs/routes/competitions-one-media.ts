// ── One handler for both kinds (Round 5 E-2): the body of /api/{leagues,clubs}/[id]/competitions/[competitionId]/media ──
...
// ── /api/{leagues,clubs}/[id]/competitions/[competitionId]/media (phase 4 R3) ───────
// The contest media library. The gate proves the caller manages THIS
// org; the lib resolves owner vs participant authority (a league is
...
export async function competitionsOneMediaRouteGET(request: NextRequest, kind: OrgKind, params: { id: string; competitionId: string }) {
  try {
    ...
    return await contestMediaGET(ctx.admin, contestId, { side: kind, orgId: ctx.id });
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError(`[CONTEST MEDIA] ${kind} GET error:`, error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function competitionsOneMediaRoutePOST(request: NextRequest, kind: OrgKind, params: { id: string; competitionId: string }) {
  try {
    const ctx = await gateAndParams(request, kind, params);
    if ('limited' in ctx) return ctx.limited;
    ...
    if (error instanceof Response) return error;
    reportRouteError(`[CONTEST MEDIA] ${kind} POST error:`, error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function competitionsOneMediaRoutePATCH(request: NextRequest, kind: OrgKind, params: { id: string; competitionId: string }) {
  try {
    const ctx = await gateAndParams(request, kind, params);
    if ('limited' in ctx) return ctx.limited;
    ...
    if (error instanceof Response) return error;
    reportRouteError(`[CONTEST MEDIA] ${kind} PATCH error:`, error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

export async function competitionsOneMediaRouteDELETE(request: NextRequest, kind: OrgKind, params: { id: string; competitionId: string }) {
  try {
    const ctx = await gateAndParams(request, kind, params);
    if ('limited' in ctx) return ctx.limited;
    ...
    if (error instanceof Response) return error;
    reportRouteError(`[CONTEST MEDIA] ${kind} DELETE error:`, error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
