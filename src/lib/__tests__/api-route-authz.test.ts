...
const GATE_RE =
  ...
  // requireActiveWriter (Support & Reporting, Spec 2) wraps requireAuth + the moderation write gate.
  /\b(requireAuth|requireActiveWriter|getServerAuth|requireAdmin|requireModerator|requireProfileRole|requireGuardianAccount|resolveActingProfile|getProfileRole|mayManagePostMedia|requireScout|openMatchWrite|CRON_SECRET|auth\.getUser)\b/;
...
const PUBLIC_ROUTES: Record<string, string> = {
  ...
  'media/org-gallery/[siteId]/[mediaId]':
    'public member-photo streamer for org sites (program 10 M2; leagues since program 12); anonymous by design but NOTHING is public by default — every request re-runs the member-photo gate (site published + org public, the manager’s pick, a public published round post, a public unsupervised author, the member’s follow-row photo_consent) before a 60s signed URL; the site is an anonymous ISR surface',
