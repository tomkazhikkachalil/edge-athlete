...
export function eventApi(eventId: string, token: string | null) {
  ...
  return {
    ...
    invite: (profileIds: string[], recorder = false, coOrganizer: { playing: boolean } | null = null) => call<{ invited: string[]; skipped: Record<string, number> }>(`${base}/participants`, { method: 'POST', body: JSON.stringify({ profile_ids: profileIds, ...(recorder ? { recorder: true } : {}), ...(coOrganizer ? { role: 'co_organizer', playing: coOrganizer.playing } : {}) }) }),
    ...
    // Phase 4 (216): the gallery — the list (polled while live), an add after uploadPostMedia, a remove.
    media: () => call<{ media: import('./media').EventMediaView[]; can_add: boolean }>(`${base}/media${q(token)}`),
    addMedia: (body: { media_url: string; media_type: 'image' | 'video'; thumbnail_url?: string | null; duration_seconds?: number | null; caption?: string | null; round_id?: string | null }) => call<{ media: import('./media').EventMediaView[]; added: string }>(`${base}/media`, { method: 'POST', body: JSON.stringify(body) }),
    removeMedia: (mediaId: string) => call<{ media: import('./media').EventMediaView[] }>(`${base}/media/${mediaId}`, { method: 'DELETE' }),
