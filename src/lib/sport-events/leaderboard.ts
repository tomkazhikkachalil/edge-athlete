...
function scorePlayer(p: LeaderboardPlayer, input: LeaderboardInput, parByHole: Map<number, number>, siByHole: Map<number, number | null>): Scored {
  ...
  if (thru === 0) return { player: p, thru: 0, gross: null, toPar: null, net: null, netToPar: null, courseHandicap: null, netReason: null };
  let gross = 0;
  let toPar = 0;
  ...
  if (p.handicapIndex === null) netReason = 'no_index';
    ...
    for (const h of scored) {
      ...
      allocated += strokesForHole(ch, si);
    }
    if (netReason === null) {
      net = gross - allocated;
  ...
  return { player: p, thru, gross, toPar, net, netToPar, courseHandicap: ch, netReason };
}
...
export function computeLeaderboard(input: LeaderboardInput): LeaderboardRow[] {
  ...
  const keyOf = (s: Scored): number | null => (useNet ? s.net : s.gross);
  const ranked = scored.filter(s => s.thru > 0 && keyOf(s) !== null);
  const unranked = scored.filter(s => !(s.thru > 0 && keyOf(s) !== null));
  ranked.sort((a, b) => {
    const ka = keyOf(a) as number;
    const kb = keyOf(b) as number;
    if (ka !== kb) return ka - kb;
    const ta = (useNet ? a.netToPar : a.toPar) as number;
    const tb = (useNet ? b.netToPar : b.toPar) as number;
    if (ta !== tb) return ta - tb;
    if (a.thru !== b.thru) return b.thru - a.thru;
    return a.player.name.localeCompare(b.player.name);
  });
  const ranks = assignSharedRanks(ranked.length, i => `${keyOf(ranked[i])}|${useNet ? ranked[i].netToPar : ranked[i].toPar}`);
