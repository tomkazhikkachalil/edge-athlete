...
export default function RoundEditWindow({ round, nextSequence, onClose, onSave }: Props) {
  ...
  const save = async () => {
    const r = validateRoundDraft(draft);
    setRefusal(r);
    if (r) return;
    setBusy(true);
    const res = await onSave(roundBodyFrom(draft));
    setBusy(false);
