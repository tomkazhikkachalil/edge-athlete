...
export default function MultiSportActivity({ profileId, onEdit, onDelete }: MultiSportActivityProps) {
  ...
  // getPrimarySports pads to ≥3 enabled sports, so [0] always exists — no
  // hardcoded golf terminal fallback.
  const [activeSportKey, setActiveSportKey] = useState<SportKey>(primarySportKeys[0] ?? getPrimarySports()[0].sport_key);
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between ${cssClasses.SPACING.MARGIN.BASE}`}>
        <div className="flex-1">
          ...
          <p className={`${cssClasses.TYPOGRAPHY.CHIP} text-muted mt-1`}>
            ...
            {getSportAdapter(activeSportKey).getNavLinks().map(link => (
              <Link key={link.href} href={link.href} className="ml-3 text-brand-fg hover:text-brand-fg-strong font-medium">
                {link.label}
              </Link>
            ))}
          </p>
