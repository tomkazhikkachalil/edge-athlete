...
function DeltaGlyph({ now, prior }: { now: number; prior: number }) {
  if (now === prior) return <Minus className="w-3.5 h-3.5 text-gray-400" aria-label="unchanged" />;
  return now > prior
...
export default function HeroStrip({ summary, streak, pb }: HeroStripProps) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
      ...
      <div className="col-span-2 bg-white rounded-lg border border-gray-200 p-4 flex items-center gap-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${pb ? accent.chip : 'bg-gray-100'}`}>
          <Trophy className={`w-5 h-5 ${pb ? accent.text : 'text-gray-400'}`} aria-hidden="true" />
        </div>
        {pb ? (
          <div className="min-w-0">
            <div className="text-xs text-gray-500 uppercase tracking-wide">Latest personal best</div>
            <div className="text-lg font-bold text-gray-900 truncate tabular-nums">
          ...
          <div className="min-w-0">
            <div className="text-xs text-gray-500 uppercase tracking-wide">Latest personal best</div>
            <div className="text-sm text-gray-500">Log a metric to start chasing PBs.</div>
          </div>
        )}
      </div>
    </div>
