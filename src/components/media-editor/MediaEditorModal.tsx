...
export default function MediaEditorModal({ assets, config, onDone, onCancel }: MediaEditorModalProps) {
  useBodyScrollLock(true);
  const { showError } = useToast();
  const defaultAspect = config.enforcedRatio ?? config.aspectRatios[0] ?? 'free';
  const { recipes, patchRecipe, previewUrls } = useEditorSession(assets, defaultAspect);
  ...
  const editable = active.kind === 'image' && !isGif;
  ...
  const handleDone = async () => {
    ...
    // Sequential on purpose — parallel decodes of large photos kill mobile tabs
    for (let i = 0; i < assets.length; i++) {
      ...
      try {
        ...
        if (isServerAllowedType(asset.file.type)) {
          // Renderable-but-failed: fall back to the original bytes
    ...
    <div className="fixed inset-0 z-[65] bg-black flex flex-col safe-top safe-bottom">
      ...
      {!activeUrl ? (
            ...
            {TOOLS.map(t => (
              <button
                ...
                className={`px-4 min-h-[44px] rounded-full text-label transition-colors ${
                  tool === t.id ? 'bg-white/20 text-white font-semibold' : 'text-white/60 hover:text-white'
                }`}
              >
                {t.label}
              </button>
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center gap-3 px-6">
          {active.kind === 'video' ? (
            <video src={activeUrl} controls playsInline className="max-w-full max-h-[60vh] rounded-lg" />
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
          ...
          <p className="text-label text-white/70 text-center">
            {active.kind === 'video'
              ? 'Video editing is coming soon — this video will upload as-is.'
