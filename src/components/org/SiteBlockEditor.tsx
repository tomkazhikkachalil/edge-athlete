...
// ── The shared block editor (phase 3 R3; parameterized for news in 3.5) ────
// One org_site_pages OR org_site_news row, its ordered block array edited
// inline — move/remove/patch-at-index (the RoutineEditorModal shape,
// rendered flat: never a modal, 375px). Save PATCHes {title, body};
// the Publish toggle PATCHes visibility (pages) or publish (news — the
// published_at timestamp IS the state). Unsaved edits get a chip + a
// beforeunload guard (App Router back-nav isn't interceptable, so no
// back-guard by design). Both console subpages are thin wrappers.
...
const MODE_COPY = {
  page: {
    ...
    unpublishLabel: 'Set to draft',
    publishedToast: 'Page is live',
    draftToast: 'Page set to draft',
    savedToast: 'Page saved',
  },
  news: {
    header: 'News editor',
    saveLabel: 'Save post',
    ...
    publishedToast: 'Post is live',
    draftToast: 'Post unpublished',
    savedToast: 'Post saved',
  },
} as const;

export default function SiteBlockEditor({ mode }: { mode: 'page' | 'news' }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setAuthorized(true);
        const page = body.page ?? body.post;
        setSlug(page.slug ?? '');
        setSiteId(page.site_id ?? '');
        setTitle(page.title ?? '');
        setVisibility(
          (mode === 'page' ? page.visibility === 'public' : Boolean(page.published_at))
            ? 'public'
            : 'draft'
        );
  ...
  const save = async () => {
    ...
    try {
      ...
      showSuccess('Website', copy.savedToast);
