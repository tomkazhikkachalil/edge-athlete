...
export default function ClubPage() {
  ...
  const countMyRounds = async () => {
    try {
      const response = await fetch(`/api/clubs/${encodeURIComponent(clubId)}/roster`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        showError('Club', body.error || 'Could not join the roster');
        return;
      }
      showSuccess('Club', body.action === 'guardian_asked' ? 'Your guardian has been asked to confirm' : 'Your rounds now count in club leagues');
      refresh();
    } catch {
      showError('Club', 'Could not join the roster');
    }
  };
  const shareJoinLink = async () => {
    const url = `${window.location.origin}/join/club/${clubId}`;
    try {
      if (typeof navigator.share === 'function') {
        await navigator.share({ title: `Join ${data?.club.name ?? 'us'}`, url });
        return;
      }
      await navigator.clipboard.writeText(url);
      showSuccess('Club', 'Join link copied');
    } catch {
      /* dismissed */
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        {user && viewerRole && viewerRole !== 'owner' && !viewerRoster && (
          <div className="mt-6 bg-surface rounded-xl shadow-sm border border-border p-4 sm:p-6" data-self-roster="offer">
            <p className="font-medium text-primary">{`Count your rounds in ${club.name} leagues`}</p>
            <p className="mt-1 text-sm text-secondary">
              {viewerProfile?.supervision_state === 'supervised'
        ...
        <OrgNewsCard side="club" orgId={club.id} isMember={!!viewerRole || isOwner} />
