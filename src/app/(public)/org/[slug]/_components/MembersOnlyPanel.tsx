...
// "Members only" (phase 9 V4; leagues in program 11) — what a private org's
// site shows in place of standings, teams, players, leaders and the
// gallery. Server-safe and
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-secondary">
        {`${what ?? 'This'} is visible to ${site.orgName} members. Join the ${site.side}, or sign in if you're already a member.`}
      </p>
      <p className="mt-3 flex flex-wrap justify-center gap-3 text-sm">
        <a
          href={`${app}/join/${site.side}/${site.orgId}`}
          className="inline-block rounded-md bg-brand px-4 py-2 font-semibold text-white"
        >
          {`Join ${site.orgName}`}
        </a>
        <a href={`${app}/?next=${encodeURIComponent(`/${site.side}/${site.orgId}`)}`} className="inline-block px-4 py-2 font-medium text-brand-fg">
          Sign in →
        </a>
      </p>
    </div>
