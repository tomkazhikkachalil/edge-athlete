...
// ── /api/leagues/[id]/structure/divisions — manager division CRUD ──────────
...
export async function POST(
  ...
  try {
    ...
    return await divisionCreatePOST(admin, parsed.data, { side: 'league', orgId: id });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    if (!gate.ok) return gate.response;
    return await divisionDELETE(admin, divisionId, { side: 'league', orgId: id });
  } catch (error) {
    if (error instanceof Response) return error;
