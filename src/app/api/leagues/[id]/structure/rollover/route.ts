...
// ── /api/leagues/[id]/structure/rollover (phase 5.5) ────────────────────────
...
export async function POST(
  ...
  try {
    ...
    return await seasonRolloverPOST(admin, 'league', id, parsed.data, user.id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ROLLOVER] league POST error:', error);
