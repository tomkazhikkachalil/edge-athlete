...
// ── /api/leagues/[id]/structure/programs — a shim (Round 5 E-2) ──
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return structureProgramsRoutePOST(request, 'league', await params);
}

export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return structureProgramsRouteDELETE(request, 'league', await params);
}
