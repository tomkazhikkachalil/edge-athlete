...
// ── /api/leagues/[id]/venues — the org's venues + golf links (phase 6b A1) ──
...
export async function GET(
  ...
  try {
    ...
    return await orgVenuesGET(getSupabaseAdmin(), { side: 'league', orgId: id });
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    return await orgVenueCreatePOST(admin, { side: 'league', orgId: id }, parsed.data);
  } catch (error) {
    if (error instanceof Response) return error;
