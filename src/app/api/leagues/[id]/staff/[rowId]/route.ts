...
// ── /api/leagues/[id]/staff/[rowId] — change / revoke one grant (league twin) ──
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; rowId: string }> }) {
  try {
    ...
    return await staffRowPATCH(request, user, 'league', params);
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; rowId: string }> }) {
  try {
    const user = await requireAuth(request);
    return await staffRowDELETE(request, user, 'league', params);
  } catch (error) {
    if (error instanceof Response) return error;
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
