...
export async function GET(
  ...
  try {
    ...
    reportRouteError('[ORG SITE PAGES] league GET error:', error);
...
export async function POST(
  ...
  try {
    ...
    return await pageCreatePOST(admin, 'league', id, parsed.data, user.id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE PAGES] league POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
