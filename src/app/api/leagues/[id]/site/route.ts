...
export async function GET(
  ...
  try {
    ...
    console.error('[ORG SITES] league GET error:', error);
...
export async function POST(
  ...
  try {
    ...
    const gate = await requireOrgManager(admin, user, 'league', id);
    if (!gate.ok) return gate.response;
    return await siteCreatePOST(admin, 'league', id, gate.org.name);
  } catch (error) {
    if (error instanceof Response) return error;
    console.error('[ORG SITES] league POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    return await sitePATCH(admin, 'league', id, parsed.data.action);
  } catch (error) {
    if (error instanceof Response) return error;
    console.error('[ORG SITES] league PATCH error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
