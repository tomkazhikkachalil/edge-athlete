...
// ── /api/leagues/[id]/site/news — news list + create (phase 3 R3) ─────────
...
export async function GET(
  ...
  try {
    ...
    reportRouteError('[ORG SITE NEWS] league GET error:', error);
...
export async function POST(
  ...
  try {
    ...
    return await newsCreatePOST(admin, 'league', id, parsed.data);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE NEWS] league POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
