...
export async function GET(
  ...
  try {
    ...
    reportRouteError('[ORG SITE NEWS] league page GET error:', error);
...
export async function PATCH(
  ...
  try {
    ...
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE NEWS] league page PATCH error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE NEWS] league page DELETE error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
