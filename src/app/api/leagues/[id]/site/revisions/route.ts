...
// ── /api/leagues/[id]/site/revisions — draft / publish / history (Site Builder
...
export async function GET(
  ...
  try {
    ...
    return await revisionsGET(admin, 'league', id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE REVISIONS] league GET error:', error);
...
export async function POST(
  ...
  try {
    ...
    const gate = await requireOrgManager(admin, user, 'league', id, { intent: 'manage_site' });
    if (!gate.ok) return gate.response;
    return await revisionsPOST(admin, 'league', id, user.id, parsed.data);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE REVISIONS] league POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
