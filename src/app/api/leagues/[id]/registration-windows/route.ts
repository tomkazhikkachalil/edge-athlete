...
// ── /api/leagues/[id]/registration-windows — a shim (Round 5 E-2) ──
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return registrationWindowsRouteGET(request, 'league', await params);
}

export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return registrationWindowsRoutePOST(request, 'league', await params);
}

export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return registrationWindowsRouteDELETE(request, 'league', await params);
}
