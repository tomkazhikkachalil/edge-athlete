...
// ── /api/leagues/[id]/competitions/[competitionId]/meet/events (track 2 PR 7) ──
...
export async function POST(
  ...
  try {
    ...
    return await meetEventsGeneratePOST(admin, parsed.data, { side: 'league', orgId: id });
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[COMPETITIONS] league meet events POST error:', error);
