...
// ── /api/leagues/[id]/competitions/[competitionId]/season-announce — a shim (Round 5 E-2) ──
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; competitionId: string }> }) {
  return competitionsOneSeasonAnnounceRouteGET(request, 'league', await params);
}

export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string; competitionId: string }> }) {
  return competitionsOneSeasonAnnounceRoutePOST(request, 'league', await params);
}
