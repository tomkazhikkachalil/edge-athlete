...
// ── /api/leagues/[id]/registrations — a shim (Round 5 E-2) ──
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return registrationsRoutePOST(request, 'league', await params);
}

export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return registrationsRouteGET(request, 'league', await params);
}
