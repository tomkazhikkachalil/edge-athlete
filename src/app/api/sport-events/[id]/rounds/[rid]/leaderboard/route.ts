...
/**
 * GET ?token=&as= — the round's leaderboard, computed on read (never
 * stored). Everyone who may view the event may read it — a private
 * event's followers included (Tom). A signed-in read caches privately for
 * 5 s; an anonymous read of a public event lets the CDN hold it 10 s.
 */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; rid: string }> }) {
  ...
  try {
    ...
    const board = await fetchRoundLeaderboard(admin, read.event, round as SportEventRoundRow);
