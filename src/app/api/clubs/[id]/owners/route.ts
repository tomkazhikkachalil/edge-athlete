...
// ── /api/clubs/[id]/owners — co-owner minting + self step-down (0.8) ──────
...
export async function POST(
  ...
  try {
    ...
    return await promoteToOwner(getSupabaseAdmin(), user, 'club', id, profileId);
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    return await stepDownAsOwner(getSupabaseAdmin(), user, 'club', id);
  } catch (error) {
    if (error instanceof Response) return error;
