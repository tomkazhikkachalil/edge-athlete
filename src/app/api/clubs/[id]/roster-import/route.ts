...
import { RosterImportSchema } from '@/lib/clubs/validate';
import { requireOrgManager } from '@/lib/orgs/structure-server';
import { importRoster, parseRosterImport, remintAthleteClaim } from '@/lib/orgs/roster-import';
import { UUID_RE } from '@/lib/golf/course-catalog';

// ── /api/clubs/[id]/roster-import — paste-import stubs / re-mint (R3) ────
...
export async function POST(
  ...
  try {
    ...
    const gate = await requireOrgManager(admin, user, 'club', id, { intent: 'manage_members' });
    ...
    if ('remintProfileId' in parsed.data) {
      ...
      if (!minted) {
        return NextResponse.json(
          { error: 'No unclaimed roster athlete with that id in this club' },
          { status: 404 }
        );
      }
