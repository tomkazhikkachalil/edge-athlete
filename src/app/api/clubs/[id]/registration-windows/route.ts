...
// ── /api/clubs/[id]/registration-windows (phase 5 R2) ─────────────────────
...
async function gate(request: NextRequest, params: Promise<{ id: string }>) {
  ...
  if (!UUID_RE.test(id)) {
    return { limited: NextResponse.json({ error: 'Club not found' }, { status: 404 }) };
  }
  const admin = getSupabaseAdmin();
  const verdict = await requireRegistrar(admin, user, 'club', id);
...
export async function GET(
  ...
  try {
    ...
    return await windowsGET(ctx.admin, 'club', ctx.id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[REGISTRATION] club windows GET error:', error);
...
export async function POST(
  ...
  try {
    ...
    return await windowCreatePOST(ctx.admin, 'club', ctx.id, parsed.data, ctx.user.id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[REGISTRATION] club windows POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return await windowDELETE(ctx.admin, 'club', ctx.id, windowId);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[REGISTRATION] club windows DELETE error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
