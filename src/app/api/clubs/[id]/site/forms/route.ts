...
// ── /api/clubs/[id]/site/forms — the site's inbox (program 2, D2) ────────
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return await formsGET(admin, 'club', id, state);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[SITE FORMS INBOX] club GET error:', error);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return await formsPATCH(admin, 'club', id, parsed.data);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[SITE FORMS INBOX] club PATCH error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
