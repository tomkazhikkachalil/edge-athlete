...
// ── /api/clubs/[id]/site/widget-data?keys=a,b — the picker's live tiles
...
export async function GET(
  ...
  try {
    ...
    return await widgetDataGET(admin, 'club', id, request.nextUrl.searchParams.get('keys'));
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[ORG SITE WIDGET DATA] club GET error:', error);
