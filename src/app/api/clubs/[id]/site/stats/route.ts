...
// ── /api/clubs/[id]/site/stats?days=7|30|90 — the site's visitors (program 2, E2)
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return await statsGET(admin, 'club', id, statsRange(request.nextUrl.searchParams.get('days')));
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[SITE ANALYTICS] club GET error:', error);
