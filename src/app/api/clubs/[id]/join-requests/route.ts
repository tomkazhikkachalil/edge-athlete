...
import { decideJoinRequest, listJoinRequests } from '@/lib/clubs/join-requests-server';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return await listJoinRequests(g.admin, g.club.id);
  } catch (error) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return await decideJoinRequest(g.admin, g.club, parsed.data.requestId, parsed.data.decision);
  } catch (error) {
    if (error instanceof Response) return error;
