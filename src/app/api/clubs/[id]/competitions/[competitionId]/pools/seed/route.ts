...
// ── /api/clubs/[id]/competitions/[competitionId]/pools/seed (leftovers PR 2) ──
...
export async function POST(
  ...
  try {
    ...
    return await poolsSeedPOST(admin, parsed.data, { side: 'club', orgId: id });
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[COMPETITIONS] club pools seed POST error:', error);
