...
// ── /api/clubs/[id]/competitions/[competitionId]/media/tags (phase 4 R3) ──
...
export async function POST(
  ...
  try {
    ...
    reportRouteError('[CONTEST MEDIA] club tags POST error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Response) return error;
    reportRouteError('[CONTEST MEDIA] club tags DELETE error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
