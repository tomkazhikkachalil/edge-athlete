...
// ── /api/clubs/[id]/competitions/[competitionId]/golf-sync/nudge (phase 8 P5) ──
...
export async function POST(
  ...
  try {
    ...
    const { data: contest } = await admin
      .from('contests')
      .select('id, competition:competition_id (id, club_id)')
      .eq('id', body.contestId)
      .maybeSingle();
    const comp = contest?.competition as { id: string; club_id: string | null } | { id: string; club_id: string | null }[] | null | undefined;
    const compRow = Array.isArray(comp) ? comp[0] : comp;
    if (!contest || !compRow || compRow.id !== competitionId || compRow.club_id !== id) {
      return NextResponse.json({ error: 'Round not found' }, { status: 404 });
    }
    return await nudgeGolfContest(admin, body.contestId);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[GOLF LEAGUE] club nudge error:', error);
