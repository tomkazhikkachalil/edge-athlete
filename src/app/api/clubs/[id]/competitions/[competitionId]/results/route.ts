...
// ── /api/clubs/[id]/competitions/[competitionId]/results (phase 2 R2) ─────
...
export async function POST(
  ...
  try {
    ...
    return await resultsUpsertPOST(admin, parsed.data, { side: 'club', orgId: id }, user.id);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[COMPETITIONS] club results POST error:', error);
