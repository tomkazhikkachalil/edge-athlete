...
// ── /api/clubs/[id]/competitions/quickstart (Onboarding v2 R4) ────────────
...
export async function POST(
  ...
  try {
    ...
    if (raw && typeof raw.sport === 'string' && raw.sport !== 'golf') {
      ...
      return await seasonQuickstartPOST(admin, user.id, { side: 'club', orgId: id }, parsedSeason.data.sport);
    }
    const parsed = await parseBody(request, GolfQuickstartSchema);
    if (!parsed.success) return parsed.response;
    return await golfQuickstartPOST(admin, user, { side: 'club', orgId: id }, parsed.data);
  } catch (error) {
    if (error instanceof Response) return error;
    reportRouteError('[COMPETITIONS] club quickstart POST error:', error);
