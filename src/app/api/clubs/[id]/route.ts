...
export async function GET(
  ...
  try {
    ...
    const listing = await readListing(supabase, 'club', id);
    ...
    return NextResponse.json({
      ...
      primarySport: listing.primarySport,
...
export async function PATCH(
  ...
  try {
    ...
    if (listingChange) {
      ...
      const applied = await applyListing(supabase, { side: 'club', orgId: id, orgName, actorId: user.id, target: listingChange });
    ...
    return NextResponse.json({
      club: updated ?? (await supabase.from('organizations').select('*').eq('id', id).maybeSingle()).data,
