-- =====================================================
-- FULL-TEXT SEARCH OPTIMIZATION (FIXED VERSION)
-- =====================================================
...
-- Function to update search vector (only uses columns that definitely exist)
CREATE OR REPLACE FUNCTION profiles_search_vector_update()
RETURNS trigger AS $$
BEGIN
  -- Build search vector from available columns
  -- Using only core columns that exist in all profiles table versions
  NEW.search_vector :=
  ...
  -- Add optional fields if they exist (will be ignored if column doesn't exist)
  BEGIN
...
-- Backfill existing profiles (this will run the trigger for each row)
UPDATE profiles SET updated_at = updated_at WHERE search_vector IS NULL;
...
UPDATE clubs SET updated_at = updated_at WHERE search_vector IS NULL;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════';
  RAISE NOTICE '    FULL-TEXT SEARCH INDEXES CREATED (FIXED)';
  RAISE NOTICE '════════════════════════════════════════════════';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  SELECT * FROM search_profiles(''john'', 10);';
  RAISE NOTICE '  SELECT * FROM search_posts(''golf tournament'', 5);';
  RAISE NOTICE '  SELECT * FROM search_clubs(''athletics'', 10);';
  RAISE NOTICE '';
  RAISE NOTICE 'NOTE: This version handles missing columns gracefully.';
  RAISE NOTICE '';
END $$;
