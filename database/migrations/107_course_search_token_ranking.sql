-- ============================================================================
-- 107: per-token ranking — proximity out of course search for good
-- ============================================================================
-- 106 swapped ts_rank_cd for ts_rank and still ranked Thunderbird Sports
-- Centre (city Kanata) above Kanata Golf Club (Kanata in the NAME) for
-- "kanata ontario". Cause, verified on prod: for a multi-term AND query
-- plain ts_rank ALSO scores term proximity (tsrank.c calc_rank_and weights
-- each pair of terms by their positional distance), and the vector is
-- name → club → city → region, so a city token beside a region token beats
-- a name token however the weights are set.
--
-- Fix: rank per TOKEN. `search_token_hits` counts the query tokens that hit
-- a vector (used on the NAME vector as a tier: "kanata ontario" → Kanata
-- Golf Club has 1 name hit, Thunderbird 0), and `search_token_rank` sums
-- single-token ts_rank values (weights only — a one-term query has no
-- proximity). Same signature, CREATE OR REPLACE, re-runnable.
-- ============================================================================
...
-- How many of the query's tokens prefix-match somewhere in `vec`.
...
-- Weight-only relevance: the sum of single-token ranks. No proximity.
...
SELECT
  (SELECT name FROM public.search_golf_courses('kanata ontario', 1)) ILIKE 'kanata%' AS kanata_name_first,
  (SELECT city FROM public.search_golf_courses('eagle', 1)) = 'Ottawa' AS eagle_ottawa_first,
  (SELECT city FROM public.search_golf_courses('eagle creek ottawa', 1)) = 'Ottawa' AS multi_token_ok,
  public.search_token_hits(to_tsvector('simple', 'kanata golf club'), 'kanata ontario') = 1 AS token_hits_ok,
