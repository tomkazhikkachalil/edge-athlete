-- ============================================================================
-- 227: Round 2 (a second environment + a rebuildable chain) — the SCHEMA DUMP
--      RPC: everything a blank project needs to become this one, as one jsonb
-- ============================================================================
-- The numbered chain does NOT replay on a blank database — proven by
-- `node scripts/chain-replay-scan.mjs` (Sep 21 2026): posts, post_media,
-- follows and athlete_equipment are altered or referenced from 002–044 but
-- created only by the Sep 14 baselines 190–191 (which recorded pre-chain
-- tables verbatim), and 003 RAISEs if `follows` is missing. So a second
-- environment cannot be built by running 001..N. It is built from the LIVE
-- schema instead — the plan's design ("generated from prod's live schema;
-- the provenance dump machinery already reads it") — and this RPC is that
-- machinery widened from "what check:schema compares" (195) to "what a
-- rebuild must emit":
--
--   extensions · enum types · sequences (with their owning column) · every
--   public table (columns with format_type, NOT NULL, default, identity,
--   generated; RLS state; comments) · constraints (pg_get_constraintdef, by
--   kind) · indexes (indexdef, flagged when a constraint owns them) · views ·
--   functions (pg_get_functiondef + acl) · triggers on public tables AND on
--   auth.users · policies on public tables AND on storage.objects · table
--   and sequence grants for the three API roles · storage buckets · pg_cron
--   jobs (bearer tokens REDACTED in the database) · the realtime
--   publication's tables · the reference rows (reserved_handles) · the
--   ledger head.
--
-- `npm run build:baseline` (scripts/build-rebuild-baseline.mjs) calls it
-- with the service key and writes database/baseline/000_rebuild.sql — the
-- file a new project runs — then re-parses that file against the live
-- OpenAPI inventory so a table or column the generator dropped is a failing
-- build, not a surprise in staging. Never the SQL editor's copy/paste: the
-- results grid is row-capped and a 120-table dump is thousands of rows.
--
-- Like 195: SECURITY INVOKER (the service role reads pg_catalog itself),
-- STABLE, service-role only. plpgsql because storage.buckets and cron.job
-- may not be readable in every project — those two sections are wrapped in
-- EXCEPTION blocks and answer `null` rather than failing the dump.
-- Re-runnable.
-- ============================================================================
...
INSERT INTO public.schema_migrations (number, name) VALUES (227, '227_schema_dump_rpc.sql') ON CONFLICT (number) DO NOTHING;

-- ── Result (ONE row; the twin under tests/diagnostics/ is the grid) ─────────
-- Expected: 227 APPLIED | true | false | <tables ≥ 120> | 227
SELECT '227 APPLIED' AS result,
       has_function_privilege('service_role', 'public.schema_dump()', 'EXECUTE') AS service_role_expect_true,
       has_function_privilege('anon', 'public.schema_dump()', 'EXECUTE') AS anon_expect_false,
       jsonb_array_length(public.schema_dump() -> 'tables') AS tables_dumped,
