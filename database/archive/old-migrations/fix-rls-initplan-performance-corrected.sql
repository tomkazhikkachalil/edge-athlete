-- ============================================
-- FIX RLS INITPLAN PERFORMANCE ISSUE (CORRECTED)
-- ============================================
...
-- Version: 1.2 (Fixed notifications column name: user_id is correct)
-- Created: 2025-01-15
-- ============================================
...
BEGIN
  ...
  -- NOTIFICATIONS (FIXED: uses user_id, not profile_id or recipient_id)
  ...
  FOR SELECT USING (
    user_id = (select auth.uid())  -- FIXED: correct column is user_id
  );
  ...
  -- NOTIFICATION_PREFERENCES (also uses user_id)
  ...
  FOR SELECT USING (
    user_id = (select auth.uid())  -- FIXED: uses user_id like notifications
  );
  ...
  RAISE NOTICE '✓ Fixed: notifications (4 policies) - uses user_id';
  RAISE NOTICE '✓ Fixed: notification_preferences (3 policies) - uses user_id';
END $$;
