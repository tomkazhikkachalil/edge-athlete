-- NOTIFICATIONS & FOLLOWERS SYSTEM
-- Implements notifications, follow messages, and connection suggestions
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  -- Reference to related entity (post_id for likes/comments, follow_id for follows)
  ...
  CONSTRAINT unique_notification_per_action UNIQUE NULLS NOT DISTINCT (recipient_id, actor_id, type, related_post_id, related_comment_id, related_follow_id)
);
...
-- =====================================================
-- PHASE 2: ADD FOLLOW MESSAGE TO FOLLOWS TABLE
-- =====================================================
...
-- Add index for follow message searches (optional)
...
BEGIN
  ...
  IF NEW.status = 'pending' THEN
    ...
    DO NOTHING; -- Prevent duplicate notifications
  END IF;
...
BEGIN
  ...
  -- Only notify if someone else liked the post (not yourself)
...
BEGIN
  ...
  -- Only notify if someone else commented (not yourself)
  IF post_owner_id IS NOT NULL AND post_owner_id != NEW.profile_id THEN
...
BEGIN
  ...
  WHERE
    ...
    AND p.visibility = 'public' -- Only public profiles
    AND (
...
SELECT proname FROM pg_proc WHERE proname LIKE '%notification%' OR proname LIKE '%suggestion%'
  ORDER BY proname;
