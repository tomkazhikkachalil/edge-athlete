...
-- result row ("210 APPLIED | 62 | 1"); the first row here names THIS file.
-- ============================================================================
...
SELECT 'type check lists 62 types', '62',
       (SELECT count(*) FROM regexp_matches(pg_get_constraintdef(oid), '''[a-z_]+''', 'g'))::text,
       CASE WHEN (SELECT count(*) FROM regexp_matches(pg_get_constraintdef(oid), '''[a-z_]+''', 'g')) = 62 THEN 'OK' ELSE 'CHECK FAILED' END
  FROM pg_constraint WHERE conname = 'notifications_type_check'
